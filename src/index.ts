...
import { UnifiedCache } from './core/cache';
  ...
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    try {
      ...
      if (request.method === 'GET' || request.method === 'HEAD') {
        ...
        // 对于 HEAD 请求，我们尽量利用 locateFile (HEAD) 的结果直接返回，减少一次 GET
        if (request.method === 'HEAD') {
            const headers = new Headers();
            headers.set('Content-Length', fileInfo.size.toString());
            headers.set('Content-Type', fileInfo.type);
            headers.set('Last-Modified', fileInfo.lastModified.toUTCString());
      ...
      if (request.method === 'PUT') {
        ...
        // 5. 后置处理
        ctx.waitUntil((async () => {
          await cluster.incrementUsage(targetBucket, size);
          await cache.setFileBucket(key, targetBucket);
        ...
        // 构造符合 S3 标准的响应：200 OK + ETag + Empty Body
        const respHeaders = new Headers();
        const etag = backendRes.headers.get('ETag');
        if (etag) respHeaders.set('ETag', etag);
        if (backendRes.headers.get('x-amz-version-id')) respHeaders.set('x-amz-version-id', backendRes.headers.get('x-amz-version-id')!);
      ...
      if (request.method === 'DELETE') {
        ...
        if (existing) {
          await cluster.deleteObject(existing.bucket, key);
          ctx.waitUntil(cache.deleteFileBucket(key));
        }
        // S3 标准：无论文件是否存在，只要请求被处理（幂等），都返回 204 No Content
        // 且 204 响应 body 必须为 null
